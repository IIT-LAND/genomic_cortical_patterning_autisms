...
libraries("here","pheatmap","reshape2","ggplot2","patchwork","Biobase","readxl","robcor")
...
pls_fname = "plsres_all_ct_GenTemp_sexAdj_MEcorr_bootCI4plotting_LV1_ci95.csv"
...
write(nz_genes, file = file.path(datapath,"nonzeromod_genes_CT_LV1.txt"))
...
write(z_genes, file = file.path(datapath,"zeromod_genes_CT_LV1.txt"))
# percentage of clustered genes falling within those modules
...
<!-- # At the level of modules, test non-zero and zero modules for enrichment in broadly expressed, brain-specific modules -->
...
```{r, warning=FALSE, message=FALSE}
ct_data = read.csv(file.path(plspath,"results","plsres_all_ct_GenTemp_sexAdj_MEcorr_bootCI4plotting_LV1_ci95.csv"))
...
ggsave(filename = file.path(plotdir, "GCLUST_CT_sexAdj_LV1_MEcorr.pdf"), width=5,height=5)
p
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
ct_allVert_data = read.csv(file.path(plspath,"results","plsres_all_ct_allVertices_sexMeanCTAdj_MEcorr_bootCI4plotting_LV1_ci95.csv"))
# sa_good = subset(sa_data,sa_data$Grp=="Good")
# sa_poor = subset(sa_data,sa_data$Grp=="Poor")
# sa_td = subset(sa_data,sa_data$Grp=="TD")

fontSize = 18
data4heatmap = ct_allVert_data
...
ggsave(filename = file.path(plotdir, "AllVertices_CT_sexMeanCTAdj_LV1_MEcorr.pdf"), width=5,height=5)
p
#------------------------------------------------------------------------------
...
ggsave(filename = file.path(plotdir, "GCLUST_AllVertices_CT_LV1_MEcorr_comp.pdf"), width=3,height=5)
r_mat
p
#------------------------------------------------------------------------------
...
df2plot$modtype = NA
df2plot$modtype[c(13,2,16,21,10,7,20,12)] = "PosNonZero"
df2plot$modtype[c(11,8,18,1)] = "NegNonZero"
...
ggsave(filename = file.path(plotdir, "plscorr_scatterplot_ASDGood_ASDPoor_CT_LV1.pdf"))
p1
...
df2plot$modtype = NA
df2plot$modtype[c(13,2,16,21,10,15,12)] = "PosNonZero"
...
p2 = p2 + xlab("TD PLS Correlation") + ylab("ASD Poor PLS Correlation") + guides(colour=FALSE, fill=FALSE) + 
  ...
  theme(text = element_text(size=fontSize), 
        ...
        axis.text.y = element_text(size=fontSize),
        plot.title = element_text(size=fontSize,hjust=0.5))
ggsave(filename = file.path(plotdir, "plscorr_scatterplot_TD_ASDPoor_CT_LV1.pdf"))
p2
...
df2plot$modtype = NA
df2plot$modtype[c(13,2,16,21,10,7)] = "PosNonZero"
df2plot$modtype[c(11,8,18,1)] = "NegNonZero"
...
p3 = p3 + xlab("TD PLS Correlation") + ylab("ASD Good PLS Correlation") + guides(colour=FALSE, fill=FALSE) + 
  ...
  theme(text = element_text(size=fontSize), 
        ...
        axis.text.y = element_text(size=fontSize),
        plot.title = element_text(size=fontSize,hjust=0.5))
ggsave(filename = file.path(plotdir, "plscorr_scatterplot_TD_ASDGood_CT_LV1.pdf"))
p3
...
for (i in 1:length(geneclasses)){
  ...
  write(genes2export, 
        file = file.path(here("results",sprintf("%s_nonzero_overlap_CTLV1.txt",geneclassnames[i]))))
...
write.csv(enrich_res_table, file = here("results","enrich_res_table_CTLV1.csv"))
```
...
ggsave(filename = file.path(plotdir,"humanspecific_enrichment_CT_LV1.pdf"))
p
...
p = p + geom_bar(stat="identity", colour="black") + 
  ...
  theme(axis.text.x = element_text(size=fs2use),
        ...
        strip.text.x = element_text(size=fs2use),
        plot.title = element_text(size=fs2use,hjust=0.5))
ggsave(filename = file.path(plotdir,"asdgenelist_enrichment_CT_LV1.pdf"))
p
...
p = p + geom_bar(stat="identity", colour="black") + 
  ...
  theme(axis.text.x = element_text(size=fs2use),
        ...
        strip.text.x = element_text(size=fs2use),
        plot.title = element_text(size=fs2use,hjust=0.5))
ggsave(filename = file.path(plotdir,"asdcelltypes_enrichment_CT_LV1.pdf"))
p
...
p = p + geom_bar(stat="identity", colour="black") + 
  ...
  theme(axis.text.x = element_text(size=fs2use),
        ...
        plot.title = element_text(size=fs2use,hjust=0.5))
  # scale_fill_gradientn(colors = colorRampPalette(c("white","red"))(100))
ggsave(filename = file.path(plotdir, "prenatalCellTypes_enrichments_CT_LV1.pdf"))
p
...
CT LV1 non-zero & Prenatal Excitatory & Prenatal PC2 (D-V) & ASD Prenatal CoExpMod

```{r, warning=FALSE, message=FALSE}
excitatory_list = unique(c(ExN,ExM,ExMU,ExDp1,ExDp2))

ex_pc2_list = excitatory_list[is.element(excitatory_list,W234_PC2)]
ex_pc2_asdprenatal_list = ex_pc2_list[is.element(ex_pc2_list,ASDPrenatal1)]

ex_pc2_asdprenatal_ctlv1_list = ex_pc2_asdprenatal_list[is.element(ex_pc2_asdprenatal_list,nz_genes)]
sort(ex_pc2_asdprenatal_ctlv1_list)

# overlap with SFARI ASD genes
ex_pc2_asdprenatal_SFARI_ctlv1_list = ex_pc2_asdprenatal_ctlv1_list[is.element(ex_pc2_asdprenatal_ctlv1_list,SFARIASD)]
sort(ex_pc2_asdprenatal_SFARI_ctlv1_list)
```
