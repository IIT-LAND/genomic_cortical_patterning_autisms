function result = pls_03_batch_PLS_all_sa_sexAdj
...
fname2save = fullfile(rootpath,'pls','results','plsres_subgrp_all_GenTemp_cortical_sa_me_sexAdj.mat');
...
for i = 1:length(result.sigLVs)
    ...
    for j = 1:length(cis2use)
        ...
        file2save = fullfile(f_path,sprintf('plsres_all_sa_GenTemp_sexAdj_MEcorr_bootCI4plotting_LV%d_ci%d.csv',LVnum,ci_bounds(2)-ci_bounds(1)));
...
writetable(tab2write, fullfile(f_path,'plsres_all_sa_GenTemp_sexAdj_brainBSR4plotting.csv'),'FileType','text','Delimiter',',');
