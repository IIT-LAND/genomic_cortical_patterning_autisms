function result = pls_03_batch_PLS_all_ct_sexAdj_SPLITHALF
...
Brain_files = {fullfile(datapath,'ASDGood_all_cortical_ct_sexAdj_HaglerGeneticTemplate.txt'), ...
    fullfile(datapath,'ASDPoor_all_cortical_ct_sexAdj_HaglerGeneticTemplate.txt'), ...
    fullfile(datapath,'TD_all_cortical_ct_sexAdj_HaglerGeneticTemplate.txt')};
nregions_per_hemi = 12; nregions_total = nregions_per_hemi*2; 

fname2save = fullfile(rootpath,'pls','results','plsres_subgrp_all_GenTemp_cortical_ct_me_sexAdj_SPLITHALF.mat');
...
for i = 1:length(result.sigLVs)
    ...
    for j = 1:length(cis2use)
        ...
        file2save = fullfile(f_path,sprintf('plsres_all_ct_GenTemp_sexAdj_MEcorr_bootCI4plotting_LV%d_ci%d_SPLITHALF.csv',LVnum,ci_bounds(2)-ci_bounds(1)));
...
writetable(tab2write, fullfile(f_path,'plsres_all_ct_GenTemp_sexAdj_brainBSR4plotting_SPLITHALF.csv'),'FileType','text','Delimiter',',');
