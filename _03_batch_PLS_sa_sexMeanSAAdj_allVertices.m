function result = pls_03_batch_PLS_all_sa_sexMeanSAAdj_allVertices
...
fname2save = fullfile(rootpath,'pls','results','plsres_subgrp_all_allVertices_cortical_sa_me_sexMeanSAAdj.mat');
...
mri_data = readtable(fullfile(datapath,'mri_data_adj_sexMeanSA_all_cortical_sa_allVertices.csv'));
...
for i = 1:length(result.sigLVs)
    ...
    for j = 1:length(cis2use)
        ...
        file2save = fullfile(f_path,sprintf('plsres_all_sa_allVertices_sexMeanSAAdj_MEcorr_bootCI4plotting_LV%d_ci%d.csv',LVnum,ci_bounds(2)-ci_bounds(1)));
...
feature_names = [repmat({'Surface Area'},lh_regions,1); repmat({'Surface Area'},rh_regions,1)];
...
writetable(tab2write, fullfile(f_path,'plsres_all_sa_allVertices_sexMeanSAAdj_brainBSR4plotting.csv'),'FileType','text','Delimiter',',');
...
brain_bsr_tab = readtable(fullfile(plspath,sprintf('plsres_all_%s_allVertices_sexMeanSAAdj_brainBSR4plotting.csv',feature)),'delimiter',',');
new_bsr_tab = readtable(fullfile(datapath,sprintf('AllGroups_all_cortical_%s_badVertices.csv',feature)),'delimiter',',');

new_bsr_tab.feature = repmat({'Surface Area'},length(new_bsr_tab.vertex_label),1);
...
writetable(new_bsr_tab, fullfile(plspath,sprintf('plsres_all_%s_allVerticesGood+Bad_sexMeanSAAdj_brainBSR4plotting.csv',feature)),'FileType','text','Delimiter',',');
