function result = batch_PLS_all_gentemp
...
Brain_files = {fullfile(datapath,'ASDGood_all_cortical_sa_ct_HaglerGeneticTemplate.txt'), ...
    fullfile(datapath,'ASDPoor_all_cortical_sa_ct_HaglerGeneticTemplate.txt'), ...
    fullfile(datapath,'TD_all_cortical_sa_ct_HaglerGeneticTemplate.txt')};
REORDER_ME = 0;
nregions_per_hemi = 24; nregions_total = nregions_per_hemi*2; 

fname2save = fullfile(rootpath,'pls','results','plsres_subgrp_all_GenTemp_cortical_sa_ct_me.mat');
...
for i = 1:length(result.sigLVs)
    ...
    for j = 1:length(cis2use)
        ...
        file2save = fullfile(f_path,sprintf('plsres_all_GenTemp_MEcorr_bootCI4plotting_LV%d_ci%d.csv',LVnum,ci_bounds(2)-ci_bounds(1)));
...
feature_names = [repmat({'Surface Area'},nregions_per_hemi,1); ...
    repmat({'Cortical Thickness'},nregions_per_hemi,1)];
hemi_names = [repmat({'LH'},nregions_per_hemi/2,1);repmat({'RH'},nregions_per_hemi/2,1);repmat({'LH'},nregions_per_hemi/2,1);repmat({'RH'},nregions_per_hemi/2,1)];
...
writetable(tab2write, fullfile(f_path,'plsres_all_GenTemp_brainBSR4plotting.csv'),'FileType','text','Delimiter',',');
